...
export default function Filter({
  ...
  const handlePayChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const rawValue = e.target.value.replace(/,/g, '').replace(/\D/g, ''); // 숫자만 추출
    const formatted = rawValue.replace(/\B(?=(\d{3})+(?!\d))/g, ','); // 3자리마다 콤마
  ...
  return (
    <div className="z-[5] mx-auto flex w-full max-w-[390px] flex-col gap-[1.5rem] rounded-[1rem] border border-[#e5e4e7] bg-white p-[1.5rem] px-[1.25rem] shadow-[0_2px_8px_0_rgba(0,_0,_0,_0.25)]">
      ...
      <div className="flex flex-col">
        ...
        <div className="my-[0.75rem] grid h-[258px] w-full max-w-[348px] grid-cols-2 gap-x-[2rem] gap-y-[2rem] overflow-x-hidden overflow-y-auto rounded-[0.6rem] border border-[#e5e4e7] px-[28px] py-[20px] text-[1.4rem]">
        ...
        <div className="my-2 flex flex-wrap gap-2">
          {address.map((option) => (
            <button
              key={option}
              type="button"
              className="flex flex-row items-center rounded-[2rem] bg-[#ffebe7] px-[0.625rem] py-[0.375rem] text-body1 leading-[1.68rem] font-extrabold text-[#ea3c12]"
